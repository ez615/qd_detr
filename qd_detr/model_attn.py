...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        self.self_attn = MultiheadAttention(hidden_dim, num_heads=8, dropout=0.1, adaptive_temp=True)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        vid_feat = vid_feat.detach().permute(1, 0, 2)       
        ...
        vid_sim = self.self_attn(vid_feat, vid_feat, vid_feat, key_padding_mask=~src_vid_mask.bool(), need_weights=True)[1]
        ...
        hs, reference, memory, memory_global = self.transformer(src, ~mask, self.query_embed.weight, pos, video_length=video_length)  # memory_global shape: [bsz, hidden_dim]
        ...
        if self.aux_loss:
            ...
            ### EDITED
            out['aux_outputs'] = [
                {'pred_logits': a, 'pred_spans': b, 'sims': vid_sim} for a, b in zip(outputs_class[:-1], outputs_coord[:-1])]
            if self.contrastive_align_loss:
    ...
    def get_similarity_map(self, src_txt, src_txt_mask, src_vid, src_vid_mask):
        ...
        if 1 in self.loss_types or 3 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                # _min, _max = torch.min(t2v_cos_sim), torch.max(t2v_cos_sim)
                # t2v_cos_sim = t2v_cos_sim - _min
                # t2v_cos_sim = t2v_cos_sim / (_max - _min)
        ...
        if 2 in self.loss_types:
            ...
            #     # for j, r in enumerate(v2v_cos_sim):
            #     #     _min, _max = torch.min(r), torch.max(r)
            #     #     v2v_cos_sim[j] = r - _min
            #     #     v2v_cos_sim[j] = r / (_max - _min)
            ...
            v2v_sims = self.self_attn(vid_feat, vid_feat, vid_feat, key_padding_mask=~src_vid_mask.bool(), need_weights=True)[1]
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, eos_coef, losses, temperature, span_loss_type, max_v_l,
                 saliency_margin=1, iou_loss_type=0, use_matcher=True, scheduling=0, save_pred=False):
