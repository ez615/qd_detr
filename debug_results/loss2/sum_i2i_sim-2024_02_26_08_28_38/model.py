    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        if 2 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                ### fill_diagonal_zeros
                # v2v_cos_sim = (vid_feat[i] @ vid_feat[i].t()).fill_diagonal_(0)
                # v2v_cos_sim = F.softmax(temp_scale * v2v_cos_sim, dim=1)
...
class SetCriterion(nn.Module):
    ...
    def loss_spans(self, outputs, targets, indices):
        ...
        if 'saliency_scores' in outputs:
            ...
            print(f'before v2v_sims: {v2v_sims}')
            v2v_sims = torch.stack([torch.tensor([clip_sim * score for clip_sim in v2v_sim]) for v2v_sim, score in zip(v2v_sims, saliency_scores)])
            print(f'after v2v_sims: {v2v_sims}\nshape: {v2v_sims.shape}')
