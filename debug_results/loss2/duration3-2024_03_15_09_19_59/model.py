...
class QDDETR(nn.Module):
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        # vid_feat = vid_feat.masked_fill(vid_feat_mask == 0, 0).detach()
        ...
        # text_feat = text_feat.masked_fill(text_feat_mask == 0, 0).detach()
        ...
        if 1 in self.loss_types or 3 in self.loss_types:
            ...
            for i in range(bsz):
                txt = text_feat[i, :L_t[i]]
                vid = vid_feat[i, :L_v[i]]

                t2v_cos_sim = F.softmax(temp_scale * (txt @ vid.t()), dim=1)
        ...
        if 2 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                v2v_cos_sim = F.softmax(temp_scale * (vid @ vid.t()), dim=1)  # [75, 75]
                ...
                oob1 = v2v_sims[i, L_v[i]:, :]
                oob2 = v2v_sims[i, :, L_v[i]:]
                print(f'oob1: {torch.unique(oob1)}\noob2: {torch.unique(oob2)}')
