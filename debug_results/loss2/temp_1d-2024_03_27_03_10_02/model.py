...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        # self.temp_scale = nn.Parameter(torch.log(torch.tensor(1/temp_factor)))
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        self.temp_scale = nn.Parameter(torch.log(torch.tensor([1/self.temp_factor] * src_vid.shape[0])))
        print(f'self.temp_factor: {self.temp_factor}\tself.temp_scale: {self.temp_scale}')
        ...
        if 1 in self.loss_types or 3 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                t2v_cos_sim = temp_scale[i] * (text_feat[i] @ vid_feat[i].t())
        ...
        if 2 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                v2v_cos_sim = temp_scale[i] * (vid_feat[i] @ vid_feat[i].t())
