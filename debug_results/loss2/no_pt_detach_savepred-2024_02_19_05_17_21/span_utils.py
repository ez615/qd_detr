...
def span_cxw_to_window(cxw_spans, clip_length=2):
    xx_span = span_cxw_to_xx(cxw_spans)
    xx_span = torch.clamp(xx_span, min=0, max=150)

    windows = torch.round(xx_span / clip_length) * clip_length
