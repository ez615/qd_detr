    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        if 2 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                self_sim = torch.diag(v2v_cos_sim)
                ### no scaling
            ...
            self_sims = torch.stack(self_sims, 0)
        ...
        out["saliency_scores"] = (torch.sum((1 - self_sims).unsqueeze(2) * self.saliency_proj1(vid_mem) * self.saliency_proj2(memory_global).unsqueeze(1), dim=-1) / np.sqrt(self.hidden_dim))
        ...
        if self.aux_loss:
            ...
            ### EDITED
            out['aux_outputs'] = [
                {'pred_logits': a, 'pred_spans': b, 'sims': sims} for a, b in zip(outputs_class[:-1], outputs_coord[:-1])]
            if self.contrastive_align_loss:
