...
def new_loss(iou_loss_types, spans1, spans2, sims, idx, durations):
    ...
    for b, i in enumerate(idx):
        src_spans[i].append(torch.round(spans1[b] * durations[i] / 2).int().tolist())
        # print(f'duration: {durations[i]}\tbefore: {spans1_b[b].int().tolist()}\tafter: {src_spans[i][-1]}')
        tgt_spans[i].append(torch.round(spans2[b] * durations[i] / 2).int().tolist())
