    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        if 1 in self.loss_types or 3 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                plt.hist(t2v_cos_sim.flatten().detach().cpu().numpy())
                plt.savefig('t2v_cos_sim_minmax_hist.png')
        ...
        if 2 in self.loss_types:
            ...
            for i in range(bsz):
                ...
                # min-max scaling
                _min, _max = torch.min(v2v_cos_sim), torch.max(v2v_cos_sim)
                v2v_cos_sim = v2v_cos_sim - _min
                v2v_cos_sim = v2v_cos_sim / (_max - _min)

                plt.hist(v2v_cos_sim.flatten().detach().cpu().numpy())
                plt.savefig('v2v_cos_sim_minmax_hist.png')
